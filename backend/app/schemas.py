...
class AIOpportunity(BaseModel):
    level: OpportunityLevel = Field(description="How suitable the task is for AI assistance.")
    message: str = Field(description="A concise explanation of where AI can help.")
...
class SkillToPractice(BaseModel):
    name: str
    category: str
    level: str
...
class AnalysisResponse(BaseModel):
    """The stable API contract consumed by the React coaching-plan view."""

    model_config = ConfigDict(populate_by_name=True)

    task: str
    ai_opportunity: AIOpportunity = Field(alias="aiOpportunity")
    recommended_workflow: list[WorkflowStep] = Field(alias="recommendedWorkflow", min_length=3, max_length=5)
